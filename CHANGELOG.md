...
- Recorded the Baxian table as `DRAFT · PHYSICAL PROTOTYPE 01`: its current Onshape URL is a mutable `/w/` workspace and anonymous metadata access returns 403, so no dimensions, hashes, derivatives, or release state are inferred from the link or screenshots.
